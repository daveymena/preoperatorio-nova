...
async function processUserImproved(user) {
  ...
  if (configuredPath) {
    if (fs.existsSync(configuredPath)) {
      launchOpts.executablePath = configuredPath;
    } else {
      const msgWarn = `⚠️ Chromium no encontrado en ${configuredPath}, usando Puppeteer default`;
  ...
  try {
    ...
    await page.evaluate(
      (u, km) => {
        ...
        kmInputs.forEach((i) => {
          ...
          if (
            name.includes("km") ||
            name.includes("kilometraje") ||
            id.includes("kilometraje")
          ) {
            i.value = km;
        ...
        document.querySelectorAll('input[type="checkbox"]').forEach((cb) => {
          ...
          const isAcceptance =
            label.toLowerCase().includes("aceptar") ||
        ...
        document.querySelectorAll('input[type="text"]').forEach((inp) => {
          if (!inp.value && !inp.readOnly && !inp.disabled) {
            ...
            if (
              ...
              name.includes("departamento") ||
              id.includes("departamento")
            ) {
      ...
      const pageWarnings = await page.evaluate(() => {
        ...
        warningElements.forEach((el) => {
          ...
          const isWarningColor =
            bgColor.includes("rgb(255, 193, 7)") || // Amarillo/Naranja
      ...
      if (pageWarnings.length > 0) {
        ...
        await page.evaluate(() => {
          ...
          const swalConfirm = document.querySelector(
            ".swal2-confirm, .swal2-ok, button.swal2-confirm",
          );
          ...
          const modalCloseBtn = document.querySelector(
            '.modal .close, .modal [data-dismiss="modal"], .modal-footer .btn-primary',
          );
          ...
          const visibleOkButtons = Array.from(
        ...
        if (freshFormData.errors.length > 0) {
          ...
          formData.errors = freshFormData.errors.filter(
            (e) => !e.includes("Error en página"),
          );
        ...
        for (const warning of pageWarnings) {
          ...
          if (
            warningText.includes("selecciona") ||
            warningText.includes("seleccione")
          ) {
            ...
            const unselectedRadios = await page.evaluate(() => {
              ...
              return Object.keys(radios).length > 0
                ? Object.keys(radios)[0]
                : null;
            });
            ...
            warningText.includes("checkbox") ||
            warningText.includes("marcar") ||
            warningText.includes("aceptar")
          ) {
            ...
            if (uncheckedCheckboxes.length > 0) {
              ...
              await page.evaluate((name) => {
                const cb = document.querySelector(
                  `input[type="checkbox"][name="${name}"], input[type="checkbox"]#${name}`,
                );
                if (cb && !cb.checked) {
                  cb.click();
                  cb.checked = true;
            ...
            warningText.includes("campo") ||
            warningText.includes("requerido")
          ) {
    ...
    await page.screenshot({
      path: `before-submit-${user.cedula}.png`,
      fullPage: true,
    });
    ...
    if (el) {
      ...
      try {
        ...
        const checkSuccess = await page
          .evaluate(() => {
            const text = document.body.innerText || "";

            // Texto de éxito explícito
            const hasExplicitSuccess =
              text.toLowerCase().includes("guardado exitosamente") ||
            ...
            // SweetAlert de éxito
            const hasSuccessSwal = !!document.querySelector(
              ".swal2-success, .swal2-success-ring",
            );

            // Alertas de éxito
            const hasSuccessAlert = !!document.querySelector(
              ".alert-success, .toast-success",
            );

            // Verificar que NO hay errores visibles
            const hasErrors = !!document.querySelector(
              ".alert-danger, .alert-error, .error, .invalid-feedback, .is-invalid",
            );

            return {
              isSuccess:
                (hasExplicitSuccess || hasSuccessSwal || hasSuccessAlert) &&
                !hasErrors,
            };
          })
        ...
        if (checkSuccess.isSuccess) {
          ...
          const pollingResult = await page
            .evaluate(async () => {
                ...
                const check = () => {
                  ...
                  const hasSuccess =
                    ...
                    !!document.querySelector(
                      ".swal2-success, .swal2-success-ring, .alert-success, .toast-success",
                    );
      ...
      if (!successFound) {
        const validationIssues = await page
          .evaluate(() => {
            const issues = [];
            document
              .querySelectorAll(
              ...
              .forEach((el) => {
                issues.push(
                  `${el.name || el.id || "?"}: ${el.validationMessage || "inválido"}`,
                );
              });
            // También buscar mensajes de error visibles
            document
              .querySelectorAll(
                '.alert-danger, .alert-error, .error, .invalid-feedback, .is-invalid, [class*="error"]',
              )
              .forEach((el) => {
                if (el.textContent?.trim())
                  issues.push(`error: ${el.textContent.trim()}`);
              });
            return issues;
          })
          .catch(() => []);
    ...
    const finalValidation = await page
      .evaluate(() => {
        const text = document.body.innerText || "";
        const swalText =
          document.querySelector(".swal2-popup")?.innerText || "";

        // Texto de éxito explícito (en toda la página y en SweetAlert)
        const fullText = (text + " " + swalText).toLowerCase();
        const hasExplicitSuccess =
          fullText.includes("guardado exitosamente") ||
    ...
    const msgCompleto = success
      ? `✅ Completado exitosamente para ${user.nombre}`
      : `❌ Completado CON ERRORES para ${user.nombre}`;
    console.log(msgCompleto);
    logToFile(msgCompleto);
    return { success, message: success ? 'exitoso' : 'con errores' };
  } catch (e) {
