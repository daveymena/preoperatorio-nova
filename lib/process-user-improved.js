...
async function processUserImproved(user) {
  ...
  try {
    ...
    while (attempt <= CONFIG.maxRetries && !formComplete) {
      ...
      // Análisis visual
      const visualAnalysis = await visualAnalyzer.analyzeScreenshot(shot);

      if (visualAnalysis) {
        const msgVisual = `  🔍 Análisis visual: ${visualAnalysis.summary}`;
        console.log(msgVisual);
        logToFile(msgVisual);

        if (visualAnalysis.isComplete) {
          const msgCompleteVisual = `  ✅ Formulario completo según análisis visual`;
          console.log(msgCompleteVisual);
          logToFile(msgCompleteVisual);
          formComplete = true;
          break;
        }

        if (visualAnalysis.emptyFields.length > 0) {
          const msgVacios = `  ⚠️ Campos vacíos detectados: ${visualAnalysis.emptyFields.join(', ')}`;
          console.log(msgVacios);
          logToFile(msgVacios);
        }

        if (visualAnalysis.suggestions.length > 0) {
          const msgSugerencias = `  💡 Sugerencias: ${visualAnalysis.suggestions.join(', ')}`;
          console.log(msgSugerencias);
          logToFile(msgSugerencias);
        }
      }
