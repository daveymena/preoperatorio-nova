      ...
      try {
        ...
        if (screenshotTaken) {
          ...
          if (!successMessageInScreenshot) {
            const msgWarning = `  ⚠️ ADVERTENCIA: Captura tomada pero NO contiene mensaje de éxito`;
            console.log(msgWarning);
            logToFile(msgWarning);
            successFound = false;
          } else {
            const msgValidated = `  ✅ VALIDADO: Captura contiene mensaje de éxito`;
            console.log(msgValidated);
            logToFile(msgValidated);
          }
        } else {
        ...
        // Verificar si hay mensaje de éxito en la captura de respaldo
        const hasSuccessInRespaldo = await page.evaluate(() => {
          const text = document.body.innerText || '';
          return text.toLowerCase().includes('guardado') ||
                 text.toLowerCase().includes('exitoso') ||
                 text.toLowerCase().includes('completado') ||
        ...
        if (hasSuccessInRespaldo) {
          const msgRespaldoOK = `  ✅ Captura de respaldo contiene mensaje de éxito`;
          console.log(msgRespaldoOK);
          logToFile(msgRespaldoOK);
          successFound = true;
        } else {
          const msgRespaldoFail = `  ❌ Captura de respaldo NO contiene mensaje de éxito`;
          console.log(msgRespaldoFail);
          logToFile(msgRespaldoFail);
          successFound = false;
        }
      }
