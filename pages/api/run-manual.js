...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(
      "📊 Usuario encontrado:",
      user ? { id: user.id, cedula: user.cedula, email: user.email } : "NO",
    );
    ...
    console.log(
      "🔐 Verificando contraseña. Esperada:",
      user.password,
      "Recibida:",
      password,
    );
    ...
    // Ejecutar con timeout y capturar resultado
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(
        () =>
          reject(new Error("Timeout: la ejecución tomó más de 120 segundos")),
        120000,
      ),
    );

    try {
      await Promise.race([processUserImproved(user), timeoutPromise]);
      return res.status(200).json({
        message: "✅ Preoperacional completado exitosamente.",
        user: {
          nombre: user.nombre,
          placa: user.placa,
          email: user.email,
      ...
      console.error("Error ejecutando usuario:", execError);
